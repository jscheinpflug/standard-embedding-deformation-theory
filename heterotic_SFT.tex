...
\end{enumerate}
As for the Yukawas, a correlator with exactly two current-type multiplets would require a nontrivial holomorphic one-point function and therefore vanishes for a marginal deformation.
...
\fe
	...
	\mu_{[\bar i}{}^{k}\,{\cal F}_{\bar j]k}
		\;\in\;\Omega^{0,2}(X,\mathrm{End}\,E),
		\fe
		likewise obtained by projecting the mixed NS--NS--NS correlator (\ref{NSNSNS3pt}) onto a bundle direction, e.g.
		\[
		\left\langle V_{\delta A_3}^{(0)}(\infty)\,V_{\mu}^{(-1)}(1)\,\overline V_{\delta A_2}^{(-1)}(0)\right\rangle
		\]
		with $V_{\mu}^{(-1)}\sim e^{-{\wt\phi}}\,\mu_{\bar i}{}^{j}\psi_{j}\,\wt\psi^{\bar i}$ the complex-structure vertex of (\ref{defmoduli}).
		whose cohomology class is the Atiyah map (\ref{eq:Atiyah_map}) that controls which complex-structure directions survive away from the standard embedding locus.
...
\fe
which packages the CFT avatar of the Atiyah/holomorphy constraint. The statement $\alpha_{E_0}=0$ at the strict standard embedding point means simply that there is no obstruction when the bundle is \emph{not} deformed. Once we turn on a small background bundle deformation with couplings $\varepsilon^{b}$ along the current-type directions $\Phi^{b}$, the leading induced lifting of complex-structure directions is linear in $\varepsilon$ and is governed by the mixed NS--NS--NS coefficient above:
\[
(\alpha_{E_\varepsilon}([Z_I]))^{a}\;\propto\;\varepsilon^{b}\,C^{a}{}_{Ib}\,,
\]
equivalently by the requirement that the corresponding CPT bracket vanish in $H^{2}(X,\mathrm{End}\,T_X)$ as in (\ref{eq:Atiyah_lin}). Subleading (cubic and higher) corrections to this leading constraint are controlled by integrated four-point functions, see section \ref{sec:4pt_selection} (and (\ref{eq:Atiyah_4pt_LV}) for a representative large-volume form).
\end{enumerate}
...
\item For the $(2,2)$-preserving moduli (the $q=0$ descendants), the perturbing operators sit in short multiplets built on chiral primaries. Their OPEs are constrained by the chiral ring, and the corresponding directions are expected to be exactly marginal, giving the usual conformal manifold of the $(2,2)$ theory.
	...
	\subsubsection*{Large-volume interpretation of some non-vanishing cases}
...
A particularly important example is the lifting of complex-structure directions by the Atiyah constraint. To leading order away from the standard embedding, turning on a small current-type background deformation $\varepsilon^{b}$ induces the Atiyah map linearly, with components proportional to $\varepsilon^{b}C^{a}{}_{Ib}$ as in (\ref{eq:Atiyah_CPT_3pt}). At subleading order in CPT (cubic and higher in the couplings), the effective mixed coefficient receives corrections governed by integrated four-point functions. For instance, the correction to $C^{a}{}_{Ib}$ linear in an additional current-type background deformation direction $\delta A_\varepsilon\in H^{1}(X,\mathrm{End}\,T_X)$ is schematically extracted from a correlator in the ``three currents + one $q=0$'' family,
...
\int d^2z~\left\langle
	...
	\right\rangle,
	\fe
	where $V_{\delta A}^{(-1)}\sim e^{-{\wt\phi}}\,J^{A'B'}\,\delta A_{\bar i}^{A'B'}(X)\,\wt\psi^{\bar i}$ and $V_{\mu}^{(-1)}\sim e^{-{\wt\phi}}\,\mu_{\bar i}{}^{j}(X)\,\psi_{j}\,\wt\psi^{\bar i}$ are the large-volume representatives of the bundle and complex-structure vertices (section 3.4), and $V^{(0)}=X\cdot V^{(-1)}$ is obtained by picture-changing. In this limit, (\ref{eq:Atiyah_4pt_LV}) reduces to the contraction of the complex-structure deformation (with $Z_I$ represented by $\mu_I$) with the first curvature variation induced by $\delta A_\varepsilon$, i.e.\ to the cohomology class $[\iota_{Z_I}\,\delta{\cal F}(\delta A_\varepsilon)]\in H^{2}(X,\mathrm{End}\,T_X)$ that appears in the linearized Atiyah constraint (\ref{eq:Atiyah_lin}).
