...
NS--R--R three-point functions compute cubic superpotential couplings $Y_{ijk}$ (section 2 and (\ref{yukawa3pt})). They are necessary for F-flatness, while exact marginality is determined by NS--NS--NS and higher-point data entering worldsheet beta functions.
			...
			This criterion applies directly to a subset of the current-type directions in our classification. Recall that a current-type multiplet has left factor a gauge current $J^{A'B'}$ and a right-moving chiral operator ${\cal O}^{\rm NS}$ in the left vacuum module. After picture-changing, the integrated insertion involves
				...
				This all-orders integrability statement is special to the abelian block. In contrast, generic holomorphic bundle deformations on Calabi--Yau threefolds can be obstructed: Thomas constructed an explicit rank-2 stable bundle with obstructed moduli \cite{Thomas:1999}, showing that there is no Bogomolov--Tian--Todorov-type theorem for vector bundles. Geometrically, the obstruction is controlled by a nontrivial cup product / commutator class in $H^2(X,\mathrm{End}\,E)$, precisely the bundle bracket entering (\ref{eq:MC_bundle}). Restricting to commuting (Cartan) directions kills this bracket identically, which explains why the Chaudhuri--Schwartz sector is universally protected; within current-type bundle directions, it is the maximal universally integrable $(0,2)$ locus.
